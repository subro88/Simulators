"""
Standalone GLTF/GLB Cam & Follower Generator
=============================================
Generates a binary .glb model of a cam & roller follower assembly with named mechanical nodes
(CamShaft, EccentricCam, RollerFollower, FollowerStem, ReturnSpring, GuideBush).
"""
...
def generate_cam_follower_glb(output_path: str = "frontend/models/cam_follower.glb"):
    components = [
        ("CamShaft", create_cylinder, (0.35, 1.8, 0.0, 20), [0.0, 0.0, -0.4], [0.0, 0.0, math.pi / 2], [0.20, 0.75, 0.95, 1.0]),
        ("EccentricCam", create_cylinder, (1.1, 0.4, 0.0, 28), [0.0, 0.3, 0.0], [0.0, 0.0, 0.0], [0.95, 0.65, 0.15, 1.0]),
        ("RollerFollower", create_cylinder, (0.35, 0.3, 0.0, 16), [0.0, 1.6, 0.0], [0.0, 0.0, math.pi / 2], [0.34, 0.85, 0.40, 1.0]),
        ("FollowerStem", create_cylinder, (0.16, 2.2, 0.0, 16), [0.0, 2.7, 0.0], [0.0, 0.0, 0.0], [0.85, 0.88, 0.92, 1.0]),
        ("GuideBush", create_cylinder, (0.45, 1.2, 0.0, 20), [0.0, 2.8, 0.0], [0.0, 0.0, 0.0], [0.22, 0.26, 0.34, 0.6]),
    ]
    ...
    gltf_dict = {
        "asset": {"version": "2.0", "generator": "Cam Follower GLB Generator"},
        "scenes": [{"name": "CamScene", "nodes": list(range(len(nodes)))}],
        "scene": 0,
        "nodes": nodes,
        "meshes": meshes,
        "materials": materials,
    ...
    print(f"Generated Cam & Follower GLB ({len(bin_buffer)} bytes) -> {output_path}")


if __name__ == "__main__":
    generate_cam_follower_glb()
