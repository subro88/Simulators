...
/* =========================================================
   ...
   v20: engraved NHIT VisualLab watermark on the beam run-out (past capacity).
...
function drawProceduralSprite(type) {
  const off = document.createElement('canvas');
  let w = 915, h = 357;
  if (type === 'blade') { w = 716; h = 20; }
  else if (type === 'v1' || type === 'v3') { w = 300; h = 200; }
  else if (type === 'v2') { w = 350; h = 200; }
  ...
  if (type === 'blade') {
    ...
    g.addColorStop(0, '#9fa9b5');
    g.addColorStop(0.5, '#eef3f8');
    g.addColorStop(1, '#68707c');
    o.fillStyle = g;
    o.fillRect(0, 4, w, 12);
    return off;
  }
  ...
  if (type === 'base') {
    const g = o.createLinearGradient(0, 0, 0, h);
    g.addColorStop(0, '#919ba6');
    g.addColorStop(0.4, '#eef3f8');
    g.addColorStop(1, '#565e68');
    o.fillStyle = g;

    // Main horizontal beam
    o.fillRect(67, 160, 800, 40);
  ...
  // Sliding Vernier Head (v1, v2, v3)
  const g = o.createLinearGradient(0, 0, 0, h);
  g.addColorStop(0, '#9fa9b5');
  g.addColorStop(0.4, '#f2f7fc');
  g.addColorStop(1, '#5b636d');
  o.fillStyle = g;

  // Sliding Head Frame
  o.fillRect(0, 150, w > 320 ? 340 : 290, 60);

  // Moveable Outside Jaw
  o.beginPath();
  o.moveTo(0, 160);
  o.lineTo(0, 340);
  o.lineTo(35, 310);
...
const shaded = {};
function shadeSprite(img, type) {
  const w = img ? img.width : 0;
  const h = img ? img.height : 0;
  if (!w || !h) {
    return drawProceduralSprite(type);
  }
...
function buildShadedSprites() {
  shaded.base = shadeSprite(imgBase, 'base');
  shaded.blade = shadeSprite(imgBlade, 'blade');
  shaded.v1 = shadeSprite(imgVernier1, 'v1');
  shaded.v2 = shadeSprite(imgVernier2, 'v2');
  shaded.v3 = shadeSprite(imgVernier3, 'v3');
}
...
// Immediate initial fallback render in case assets fail to trigger load
setTimeout(() => {
  if (!shaded.base) {
    buildShadedSprites();
    render();
...
function drawCaliper() {
  ...
  if (!shaded.base) buildShadedSprites();
  ...
  //     Drawn BEFORE the sliding head so the head covers it like a real engraving.
  {
    ...
    ctx.fillText('NHIT VisualLab', wmX, wmY + 1);
    ctx.fillStyle = 'rgba(70,82,96,0.50)';       // recessed dark engrave on top
    ctx.fillText('NHIT VisualLab', wmX, wmY);
    ctx.restore();
  }
