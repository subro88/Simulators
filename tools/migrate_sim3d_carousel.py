"""Inject the canonical .sim3d component (canvas + right-side component carousel) into every
tool page. Regenerates index.html from v1.html:
 - If v1.html already has #model3d-wrapper, replace that native block with the canonical one
   (override: the GLB carousel viewer now owns the canvas).
 - Otherwise inject the canonical block before </body>.
v1.html is left untouched (pristine V1 backup).
"""
import re, pathlib

ROOT = pathlib.Path(__file__).resolve().parent.parent
BASE = ROOT / "nhitvisuallab" / "tools"
SB = (ROOT / "nhitvisuallab" / "shared" / "sidebar" / "sidebar.js").read_text()
TITLE_MAP = {s: t.replace('\\"', '"') for s, t in re.findall(r'"slug"\s*:\s*"([^"]+)"\s*,\s*"title"\s*:\s*"([^"]+)"', SB)}
THREE = (
...
NATIVE = re.compile(r'<(div|section) id="model3d-wrapper"[^>]*>.*?</\1>', re.S | re.I)
# Legacy V1 pages sometimes ship their own three.js (e.g. ../../shared/three3d/three.min.js)
# AFTER our CDN includes; it clobbers window.THREE and removes OrbitControls/GLTFLoader.
# Drop any <script src> that mentions "three" but is NOT our CDN hosts.
THREE_SCRIPT = re.compile(r'<script[^>]*src="([^"]*(?:three|diff3d)[^"]*)"[^>]*>\s*</script>', re.I)
CDN_HOSTS = ("cdnjs.cloudflare.com", "cdn.jsdelivr.net")
def strip_legacy_three(html):
    def repl(m):
        src = m.group(1)
        return m.group(0) if any(h in src for h in CDN_HOSTS) else ""
    return THREE_SCRIPT.sub(repl, html)
...
def main():
    ...
    for d in sorted(BASE.iterdir()):
        if not d.is_dir():
            continue
        v1 = d / "v1.html"; idx = d / "index.html"
        if not v1.exists():
            print("no v1.html, skip:", d.name); continue
        html = v1.read_text(encoding="utf-8")
        html = strip_legacy_three(html)
        title = TITLE_MAP.get(d.name, d.name.replace("-", " ").title())
        model = d.name.replace("-", "_")
        has_native_tabs = 'class="controls-bar"' in html
        block = BLOCK.format(TITLE=title, SLUG=d.name, MODEL=model, THREE=THREE)
        if has_native_tabs:
            # Native mode system toggles #model3d-wrapper via .hidden; start hidden so the
            # 3D only appears when its "3D Model" pill is clicked (matches utm-testing).
            block = block.replace('<div id="model3d-wrapper">', '<div id="model3d-wrapper" class="hidden">')
        if 'id="model3d-wrapper"' in html:
            new, n = NATIVE.subn(block, html, count=1)
            if n:
                idx.write_text(new, encoding="utf-8"); repl += 1; continue
            # matched string but regex failed -> inject before </body> as fallback
            if re.search(r'</body>', html, re.I):
                html = re.sub(r'</body>', block + "</body>", html, count=1, flags=re.I)
            else:
                html += block
            idx.write_text(html, encoding="utf-8"); inj += 1; skip += 1; continue
        # no native sim3d block -> inject
        if re.search(r'</body>', html, re.I):
            html = re.sub(r'</body>', block + "</body>", html, count=1, flags=re.I)
        else:
            html += block
        idx.write_text(html, encoding="utf-8"); inj += 1
    print(f"replaced native sim3d: {repl}; injected (no native): {inj}; of which fallback-injected: {skip}")
